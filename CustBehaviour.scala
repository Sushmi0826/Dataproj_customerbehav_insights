...
object CustBehaviour extends Serializable{
  def main(args: Array[String]): Unit = {
    ...
    //click.createOrReplaceTempView("click1")
    ...
    //newcustomer.write.options(Map("header" -> "true", "delimiter" -> ",")).csv("Inputdata/newcustomer.csv")
    //newpurchase.write.options(Map("header" -> "true", "delimiter" -> ",")).csv("Inputdata/newpurchase.csv")
    //newclick.write.options(Map("header" -> "true", "delimiter" -> ",")).csv("Inputdata/newclick.csv")
    new_customer.write
